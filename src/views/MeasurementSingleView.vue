<template>
 ...
 <div class="pl-12 pr-12 pb-0 pt-6 flex justify-between">
    <div class="bg-zinc-900" style="width: 100%">
      <h1 class="p-2 pl-6 font-bold bg-zinc-800 uppercase text-white text-left text-3xl">Single-Loadcell-PANEL</h1>
    </div>
    <!-- Export Button -->
    <button style="width: 20%;"
      class=" bg-orange-400 hover:bg-red-400 text-white py-2 px-4 rounded-none text-center"
      @click="singleCellStore.exportModal = true">
    ...
    <Dialog v-model:visible="singleCellStore.detailsModal" modal :style="{ width: '80%' }" class="text-base" :draggable="false">
        <template #header>
          <div class="flex items-center justify-between py-2 text-white rounded-t-md">
            <h3 class="text-3xl">Single Cell: {{ singleCellStore.singleCell.id }}</h3>
          </div>
        </template>
...
onMounted(() => {
  // Send View Command
  if (connection) {
    frontEndCommand(connection, FrontEndCommand.MeasurementSingleView);
    listenToWebsocket();
  }
})
