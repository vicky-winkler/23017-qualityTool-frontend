...
const frontEndNackCommand = (connection, commandID) => {
  const oscLib = new osc.simpleOscLib();
  const bufferBytes = oscLib.buildMessage({
    address: '/FRONT/NACK/',
    args: [
...
// Int Array
const frontEndIntArrayData = (connection, commandID, value, x, y) => {

  // int to blob => TODO Tests
  var blob = Buffer.from(value);
  ...
  const bufferBytes = oscLib.buildMessage({
    ...
    args: [
      { type: 'integer', value: -1 },  // Teststation ID
      { type: 'integer', value: commandID },    // CommandID
      { type: 'integer', value: OscDataType.Int },    // DataType
      { type: 'integer', value:  x},    // X
      { type: 'integer', value:  y},    // Y
      { type: 'blob', value: blob} //push value as byteArray
    ],
  });
...
// back end Settings Data
const backEndSettingsData = (connection, commandID, value) => {
  const oscLib = new osc.simpleOscLib();
  const bufferBytes = oscLib.buildMessage({
    address: '/BACK/DATA/',
    args: [
